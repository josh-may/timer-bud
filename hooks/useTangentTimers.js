...
export const useTangentTimers = () => {
  ...
  useEffect(() => {
    ...
    tangentAlarmRef.current = new Audio(process.env.NEXT_PUBLIC_TANGENT_ALERT_URL || process.env.NEXT_PUBLIC_ALARM_SOUND_URL || '/tangent-alert.mp3');
    tangentAlarmRef.current.addEventListener('error', (e) => {
      console.log('Tangent alert sound not found, using default alarm');
      tangentAlarmRef.current = new Audio(process.env.NEXT_PUBLIC_ALARM_SOUND_URL);
    });
  }, []);
        ...
        try {
          ...
          const restoredTimers = parsedTimers.map(timer => {
            ...
            return {
              ...timer,
              remainingSeconds,
              isRunning: remainingSeconds > 0 && timer.isRunning && !timer.isPaused,
              lastUpdate: now
            };
          }).filter(timer => timer.remainingSeconds > 0 || timer.isRunning === false);
  ...
  const dismissCompletedTimer = useCallback((id) => {
    stopTangentTimer(id);
  }, [stopTangentTimer]);
          ...
          if (timer.isRunning && !timer.isPaused && timer.remainingSeconds > 0) {
            ...
            if (newRemaining === 0) {
              if (tangentAlarmRef.current) {
                tangentAlarmRef.current.play().catch(e => console.log('Tangent alarm play failed:', e));
              }
  ...
  const activeTangentTimers = tangentTimers.filter(timer => timer.remainingSeconds > 0 || (timer.remainingSeconds === 0 && timer.isRunning === false));
